-- Add is_demo flags
ALTER TABLE public.residences ADD COLUMN IF NOT EXISTS is_demo boolean NOT NULL DEFAULT false;
ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS is_demo boolean NOT NULL DEFAULT false;
CREATE INDEX IF NOT EXISTS idx_residences_is_demo ON public.residences(is_demo);
CREATE INDEX IF NOT EXISTS idx_leads_is_demo ON public.leads(is_demo);

CREATE OR REPLACE FUNCTION public.ensure_demo_org()
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE oid uuid;
BEGIN
  SELECT id INTO oid FROM public.organizations WHERE slug = 'demo-org' LIMIT 1;
  IF oid IS NULL THEN
    INSERT INTO public.organizations(name, slug, contact_email, contact_phone)
    VALUES ('Démo Bruxelles', 'demo-org', 'demo@example.be', '+32 2 000 00 00')
...
BEGIN
  ...
  SELECT array_agg(id) INTO res_ids FROM public.residences WHERE is_demo = true;
  IF res_ids IS NOT NULL THEN
    DELETE FROM public.pricing WHERE unit_type_id IN (SELECT id FROM public.unit_types WHERE residence_id = ANY(res_ids));
    DELETE FROM public.unit_types WHERE residence_id = ANY(res_ids);
    DELETE FROM public.residence_services WHERE residence_id = ANY(res_ids);
    DELETE FROM public.residence_activities WHERE residence_id = ANY(res_ids);
    DELETE FROM public.photos WHERE residence_id = ANY(res_ids);
    DELETE FROM public.residence_versions WHERE residence_id = ANY(res_ids);
    DELETE FROM public.residence_events WHERE residence_id = ANY(res_ids);
    DELETE FROM public.favorites WHERE residence_id = ANY(res_ids);
    DELETE FROM public.leads WHERE residence_id = ANY(res_ids) OR is_demo = true;
    DELETE FROM public.residences WHERE id = ANY(res_ids);
    n_res := array_length(res_ids, 1);
  END IF;
  INSERT INTO public.audit_log(actor_id, action, entity, entity_id, metadata_json)
  VALUES (uid, 'DEMO_PURGE', 'residences', NULL, jsonb_build_object('residences', n_res));
  RETURN jsonb_build_object('residences_deleted', COALESCE(n_res, 0));
END $$;
...
DECLARE
  ...
  type_codes text[] := ARRAY['services','seigneurie','repos','repos_soins'];
...
BEGIN
  ...
  FOR i IN 1..25 LOOP
    INSERT INTO public.residences(
      ...
      (type_codes[1 + (i % 4)])::residence_type,
    ...
    FOR j IN 1..(2 + (i % 3)) LOOP
      ...
      INSERT INTO public.pricing(
        ...
        utid, 'rent'::occupation_mode, rent_min, rent_max,
        charges, pack, '[]'::jsonb,
    ...
    VALUES (rid, 'https://picsum.photos/seed/demo' || i || '-cover/1200/800', 'cover'::photo_category,
    ...
    FOR j IN 1..(7 + (i % 5)) LOOP
      ...
      VALUES (
        ...
        (photo_cats[1 + (j % 7)])::photo_category,
        'Photo ' || j,
        'Vue ' || j || ' de ' || names[i],
        j
      );
