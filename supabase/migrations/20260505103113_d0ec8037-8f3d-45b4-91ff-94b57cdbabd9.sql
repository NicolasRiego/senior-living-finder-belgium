...
DECLARE
  ...
  photo_cats text[] := ARRAY['exterior','common_area','apartment','dining','garden','medical','activity','other'];
  ...
  FOR i IN 1..total_count LOOP
    ...
    FOR j IN 1..(6 + (i % 6)) LOOP
      ...
      VALUES (
        ...
        (photo_cats[1 + (j % 8)])::public.photo_category,
        'Photo ' || j,
        'Vue ' || j || ' de ' || names[i],
        j
      );
...
GRANT EXECUTE ON FUNCTION public.unit_type_residence(uuid) TO anon, authenticated;